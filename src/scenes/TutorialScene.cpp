...
void TutorialScene::onFinishWriting() {
  switch (state) {
...
void TutorialScene::onConfirmedOption(int option) {
  switch (state) {
    case CalibrationState::INTRO: {
      if (option == 0) {  // Yes
        closeMenu();
...
void TutorialScene::onError(CalibrationError error) {
  switch (error) {
...
void TutorialScene::showInstructions() {
  state = INSTRUCTIONS;
...
void TutorialScene::saveAndGoToGame() {
